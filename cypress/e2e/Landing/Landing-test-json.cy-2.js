...
/*    beforeEach(() => {
    ...
    it('перейти на страницу /learning-center , заполнить форму и проверить Лид', function () {
        ...
        cy.xpath("//input[placeholder()='Name *']", { timeout: 10000 }).should('be.visible').type('QA_TEST');
        cy.wait(500);
        //cy.xpath("//span[text()='Фамилия']", { timeout: 10000 }).should('be.visible').next().type('QA_TEST');
        //cy.wait(500);
        cy.xpath("//input[text()='Email *']", { timeout: 10000 }).should('be.visible').type(fake_unique_email);
        cy.wait(500);
        cy.xpath("//span[text()='Телефон *']", { timeout: 10000 }).should('be.visible').type('1234567890');
        cy.wait(500);

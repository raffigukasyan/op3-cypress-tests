describe("US.1 Add User", () => {
    ...
    it('Add user', function () {
        ...
        cy.wait(500);
        cy.xpath("//div[text()='Маркетинг']").click();
        cy.contains("Выбрать: Отделы").parent().parent().last().click();
        cy.wait(500);
    ...
    it('check add User', () => {
        cy.contains('qaUserManual0.492022085662037@mail.ru').click()
    })

    it('edite User', () => {
        cy.contains('qaUserManual0.492022085662037@mail.ru').parent().parent().last().find('div').first().click();
        cy.wait(1500);
        ...
        cy.xpath('//input[@id="avatar"]').selectFile('cypress/image/editQaUser.avif', {force: true});
        cy.xpath("//span[text()='Активность']").next().children().click();
